...
def api_login(user_id):
    try:
        resp = requests.post(f"{SERVER_URL}/login", json={"user_id": user_id})
...
class MarketBankApp:
    ...
    def giris_ekrani(self):
        ...
        self.entry_user.insert(0, "WORKER_1") # Test için
    ...
    def kart_tanit_penceresi(self):
        kart_id = simpledialog.askstring("Kart Tanıt", "Yeni Kartı Okutun (ID):")
        if not kart_id: return
